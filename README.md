...
# Ищем только в путях
docker compose -f docker-compose.app.yml run --rm -e VAULT_ADDR=http://host.docker.internal:8200/ -e VAULT_TOKEN=токен vault-search "что_ищем" --mode path

# Ищем только во внутренних ключах
docker compose -f docker-compose.app.yml run --rm -e VAULT_ADDR=http://host.docker.internal:8200/ -e VAULT_TOKEN=токен vault-search "что_ищем" --mode keys
